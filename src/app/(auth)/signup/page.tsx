...
import { Camera, UserCheck, AlertTriangle, Loader, Eye, EyeOff, Sparkles, Shuffle } from 'lucide-react';
    ...
    verificationTimeoutRef.current = setTimeout(() => {
        ...
        if (formData.gender && formData.gender !== 'other' && formData.gender !== mockDetectedGender) {
            ...
            setTimeout(() => {
                ...
                setFailureReason(null);
                setStep(2); // Cinsiyet adımına geri dön
            }, 5000);
            return;
        }
        ...
        {step === 3 && (
            <div className="grid gap-4">
                ...
                <div className='flex justify-between items-center'>
                    ...
                    <Button type="button" variant="link" size="sm" onClick={generateStrongPassword} className="gap-1 p-0 h-auto">
                        <Sparkles className="w-3 h-3"/> Öneri
                    </Button>
                </div>
                <div className="grid gap-2">
        ...
        {step === 4 && (
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="flex gap-4 p-4 rounded-lg bg-muted">
              {CAPTCHA_OPTIONS.map((opt) => (
                <button 
                    ...
                    className={cn(
                        ...
                        captchaSelected === opt.name ? 'bg-primary scale-110' : 'hover:bg-primary/20'
                    )}
                >
                    {opt.icon}
                </button>
             ...
             <Button type="button" variant="link" size="sm" onClick={shuffleCaptcha} className="gap-1">
                <Shuffle className="w-3 h-3"/> Farklı Sor
            </Button>
          </div>
        )}
        {step === 5 && (
